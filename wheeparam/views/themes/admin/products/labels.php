...
<div class="grid">
    <table style="table-layout: fixed">
        ...
        <tr>
            ...
            <th class="W80">BEST</th>
            <th class="W80">HIT</th>
            <th class="W80">NEW</th>
            <th class="W80">MD추천</th>
            <th class="W80">할인</th>
            <th class="W80">관리</th>
        </tr>
        </thead>
        ...
        <tr>
            ...
            <td class="text-center">
                <label class="w-check" style="margin:0;">
                    <input type="checkbox" value="Y" name="prd_is_best[<?=$row['prd_idx']?>]" <?=$row['prd_is_best']==='Y'?'checked':''?>>
                    <span></span>
                </label>
            </td>
            <td class="text-center">
                <label class="w-check" style="margin:0;">
                    <input type="checkbox" value="Y" name="prd_is_hit[<?=$row['prd_idx']?>]" <?=$row['prd_is_hit']==='Y'?'checked':''?>>
                    <span></span>
                </label>
            </td>
            <td class="text-center">
                <label class="w-check" style="margin:0;">
                    <input type="checkbox" value="Y" name="prd_is_new[<?=$row['prd_idx']?>]" <?=$row['prd_is_new']==='Y'?'checked':''?>>
                    <span></span>
                </label>
            </td>
            <td class="text-center">
                <label class="w-check" style="margin:0;">
                    <input type="checkbox" value="Y" name="prd_is_recommend[<?=$row['prd_idx']?>]" <?=$row['prd_is_recommend']==='Y'?'checked':''?>>
                    <span></span>
                </label>
            </td>
            <td class="text-center">
                <label class="w-check" style="margin:0;">
                    <input type="checkbox" value="Y" name="prd_is_sale[<?=$row['prd_idx']?>]" <?=$row['prd_is_sale']==='Y'?'checked':''?>>
                    <span></span>
                </label>
            </td>
            <td class="text-center">
...
<script>
    $(function() {
        $('[data-form="form-stocks"]').on('submit', function(e) {
            ...
            $.ajax({
                url: base_url + '/admin/ajax/products/labels',
                type: 'POST',
                data: data,
                success:function() {
                    location.reload();
