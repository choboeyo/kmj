...
CREATE VIEW `wb_products_category_list` AS
(
select `PC`.`cat_id`                                                                              AS `cat_id`,
       `PC`.`cat_parent_id`                                                                       AS `cat_parent_id`,
       `PC`.`cat_status`                                                                          AS `cat_status`,
       `PC`.`cat_sort`                                                                            AS `cat_sort`,
       `PC`.`cat_title`                                                                           AS `cat_title`,
       `PC`.`cat_skin`                                                                            AS `cat_skin`,
       `PC`.`cat_skin_m`                                                                          AS `cat_skin_m`,
       `PC`.`cat_use_paging`                                                                      AS `cat_use_paging`,
       `PC`.`cat_page_rows`                                                                       AS `cat_page_rows`,
       `PC`.`cat_product_count`                                                                   AS `cat_product_count`,
       `PC`.`reg_user`                                                                            AS `reg_user`,
       `PC`.`reg_datetime`                                                                        AS `reg_datetime`,
       `PC`.`upd_user`                                                                            AS `upd_user`,
       `PC`.`upd_datetime`                                                                        AS `upd_datetime`,
       concat(case when `PC2`.`cat_id` is null then '' else lpad(`PC2`.`cat_id`, 3, '0') end,
              case when `PC1`.`cat_id` is null then '' else lpad(`PC1`.`cat_id`, 3, '0') end,
              lpad(`PC`.`cat_id`, 3, '0'))                                                        AS `node_path`,
       concat(case when `PC2`.`cat_id` is null then '' else concat(`PC2`.`cat_title`, ' > ') end,
              case when `PC1`.`cat_id` is null then '' else concat(`PC1`.`cat_title`, ' > ') end) AS `parent_names`
from ((`wb_products_category` `PC` left join `wb_products_category` `PC1`
       on (`PC1`.`cat_id` = `PC`.`cat_parent_id`)) left join `wb_products_category` `PC2`
      on (`PC2`.`cat_id` = `PC1`.`cat_parent_id`)));