...
insert  into `wb_config`(`cfg_key`,`cfg_value`) values
  ...
  ('email_send_address','jang@wheeparam.com'),
  ('faq_description','사이트 이용시 자주 묻는 질문들을 쉽게 찾을 수 있습니다'),
  ('faq_title','자주 묻는 질문'),
  ('icode_userid','sir_tjsrms'),
  ('icode_userpw','jang7668!@'),
  ('naver_syndication_key',''),
  ('point_member_login','5'),
  ('point_member_register','20'),
  ('point_name','포인트'),
  ('point_use','Y'),
...
CREATE TABLE `wb_popup` (
  ...
  `pop_width` int(10) unsigned NOT NULL,
  `pop_height` int(10) unsigned NOT NULL,
  `pop_content` longtext NOT NULL,
  `pop_status` enum('Y','H','N') NOT NULL DEFAULT 'Y',
  `pop_start` datetime NOT NULL,
  `pop_type` enum('Y','N') NOT NULL DEFAULT 'Y',
  `pop_end` datetime NOT NULL,
  `pop_regtime` datetime NOT NULL,
  `pop_modtime` datetime NOT NULL,
  PRIMARY KEY (`pop_idx`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `wb_statics` (
  ...
  `sta_country` varchar(80) NOT NULL DEFAULT '',
  `sta_country_code` varchar(10) NOT NULL DEFAULT '',
  `sta_addr` varchar(80) NOT NULL DEFAULT '',
  `sta_org` varchar(255) NOT NULL DEFAULT '',
  `sta_agent` varchar(255) NOT NULL DEFAULT '',
  PRIMARY KEY (`sta_idx`),
  KEY `sta_ip` (`sta_ip`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8;
