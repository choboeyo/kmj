...
{
    $source_file = FCPATH . $filename;

    // 원본 파일이 없다면
    if (is_file($source_file) === false) return '';

    // 썸네일 크기가 지정안되있따면
    if (empty($thumb_width) && empty($thumb_height)) return base_url($filename);

    // gif, jpg, png만 적용한다.
    $size = @getimagesize($source_file);
    if ($size[2] < 1 OR $size[2] > 3) return base_url($filename);

    // Cache 디렉토리 생성
    $uploadDir = DIR_UPLOAD . '/' . 'cache' . '/';
    make_dir($uploadDir, FALSE);

    $exp = explode('/', $filename);
    $filepos = count($exp) - 1;
    for ($k = 0; $k < $filepos; $k++) {
        if($exp[$k] == DIR_UPLOAD) continue;
        $uploadDir .= $exp[$k] . '/';
    ...
    if ( ! (is_dir(FCPATH . $target_path) && is_writable(FCPATH . $target_path))) return base_url( $filename );
    ...
    $thumb_time = @filemtime(FCPATH . $thumb_file);
    $source_time = @filemtime( $source_file);

    if (file_exists(FCPATH . $thumb_file)) {
        if ($is_create === false && $source_time < $thumb_time) {
            return base_url($thumb_file);
        }
    }
